...
class Via3Json(object):
    ...
    def loadOptidsFromAtt(self,att, opts_name):
        assert isinstance(opts_name,(list,tuple,str))
        if type(opts_name) == str:
            opts_name = [opts_name]
        ids = []
        for opt_name in  opts_name:
            for opt_key in att['options']:
                if opt_name == att['options'][opt_key]:
                    ids.append(opt_key)
        return ids
    ...
    def loadMetadataInfoFromVid(self, vid, arrs_opts=None):
        assert isinstance(arrs_opts, (list, tuple)) or (arrs_opts == None)
        if arrs_opts==None:
            return self.vid2metadatas[vid]

        for i, metadata in enumerate(self.vid2metadatas[vid]):
            vid, flg = metadata['vid'], metadata['flg']
            z, xy, av = metadata['z'], metadata['xy'], metadata['av']
            filter_av = {}
            for arr_id in arrs_opts:
                if av.get(arr_id, False):
                    all_opts = set(av[arr_id].split(','))
                    valid_opts = set(arrs_opts[arr_id])
                    opts_str = ''
                    for opt in (all_opts & valid_opts):
                        if not opts_str:
                            opts_str = opts_str + opt
                        else:
                            opts_str = opts_str + ',' + opt
                    filter_av[arr_id] = opts_str
            self.vid2metadatas[vid][i]['av'] = filter_av

        return self.vid2metadatas[vid]

    def loadMetadatasInfoFromAll(self, arrs_opts=None):
        assert isinstance(arrs_opts,(dict,)) or (arrs_opts==None)
        if arrs_opts==None:
            return self.vid2metadatas

        for vid in self.vid2metadatas:
            metadata_list = self.vid2metadatas[vid]
    ...
    def dumpPrejects(self, vid_list, pid='__VIA_PROJECT_ID__', rev='__VIA_PROJECT_REV_ID__',
                     ...
                     creator='VGG Image Annotator (htt…ac.uk/~vgg/software/via)',
    ...
    def dumpAttributes(self, attributes_dict):
        ...
        for i, attribute_key in enumerate(attributes_dict, 1):
            ...
            assert attribute.get('type',False) and 1<=attribute['type']<=5
    ...
    def dumpFiles(self, files_dict):
        ...
        for i, files_key in enumerate(files_dict, 1):
            ...
            if not file_dict.get('src', False):   #file location { LOCAL:1, URIHTTP:2, URIFILE:3, INLINE:4 }
                file_dict['src'] = ''
...
if __name__ == '__main__':
    ann_file=''
    via3 = Via3Json(ann_file)
    attribute = ['person']
    custom_classes = ['flag']
    att_ids = via3.loadIdsFromAttsname(attribute)
    assert len(att_ids) > 0
    att_ids2att_labels = {att_id: i for i, att_id in enumerate(att_ids)}
    att_ids2att_infos = {att_id: via3.loadAttFromId(att_id) for att_id in att_ids}
    fids = via3.loadFilesFid()
    filesinfo = via3.loadFilesInfoFromAll()

    opt_ids2opt_names = via3.attributes[att_ids[0]]['options']
    att_ids2att_infos = via3.loadAttsFromAll()

    opt_ids = via3.loadOptidsFromAtt(att_ids2att_infos[att_ids[0]], custom_classes)
    opt_ids2opt_labels = {opt_id: label for label, opt_id in enumerate(opt_ids)}
    opt_labels2opt_names = {opt_ids2opt_labels[opt_id]:
                                     opt_ids2opt_names[opt_id]
                                 for opt_id in opt_ids}

    arrs_opts = {att_ids[0]: opt_ids}
    metadatasinfo = via3.loadMetadatasInfoFromAll(arrs_opts)